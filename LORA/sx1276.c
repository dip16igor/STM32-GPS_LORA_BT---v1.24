...
void SX1276Init( void )
{
    ...
    SX1276 = ( tSX1276* )SX1276Regs; 		// структура с регистрами FSK
    SX1276LR = ( tSX1276LR* )SX1276Regs;	// структура с регистрами Lora

    SX1276InitIo( ); // инициализация портов. Делаем ничего
    
    SX1276Reset( );  // дергаем Reset на 1 мс, пауза 6 мс
    ...
    LoRaOn = true;  				// режим Lora
    SX1276SetLoRaOn( LoRaOn );

    // Initialize LoRa modem
    SX1276LoRaInit( );  // настройки LORA модема
...
{
    if( LoRaOnState == enable )  // режим уже Lora
    {
        return;
    }
    LoRaOnState = enable;   // текущее состояние
    LoRaOn = enable;		// новое состояние

    if( LoRaOn == true )    // Включаем режим Lora
    {
   // 	OnTest();

        SX1276LoRaSetOpMode( RFLR_OPMODE_SLEEP );  // включаем SLEEP
        
        SX1276LR->RegOpMode = ( SX1276LR->RegOpMode & RFLR_OPMODE_LONGRANGEMODE_MASK ) | RFLR_OPMODE_LONGRANGEMODE_ON;  // в регистре RegOpMode меняем режим на Lora
        SX1276Write( REG_LR_OPMODE, SX1276LR->RegOpMode ); // записываем в модуль новое значение из структуры SX1276LR
        
        SX1276LoRaSetOpMode( RFLR_OPMODE_STANDBY ); // переключаемся на STANDBY
                                        // RxDone               RxTimeout                   FhssChangeChannel           CadDone
        SX1276LR->RegDioMapping1 = RFLR_DIOMAPPING1_DIO0_00 | RFLR_DIOMAPPING1_DIO1_00 | RFLR_DIOMAPPING1_DIO2_00 | RFLR_DIOMAPPING1_DIO3_00;  // настройка DIO
                                        // CadDetected          ModeReady
        SX1276LR->RegDioMapping2 = RFLR_DIOMAPPING2_DIO4_00 | RFLR_DIOMAPPING2_DIO5_00;

        SX1276WriteBuffer( REG_LR_DIOMAPPING1, &SX1276LR->RegDioMapping1, 2 );  // запись в модуль из структуры SX1276LR
        
        //SX1276ReadBuffer( REG_LR_OPMODE, SX1276Regs + 1, 0x70 - 1 );  // зачем-то читаем все регистры из модуля в SX1276Regs
    }
    else
    {
        SX1276LoRaSetOpMode( RFLR_OPMODE_SLEEP );  // включаем SLEEP
