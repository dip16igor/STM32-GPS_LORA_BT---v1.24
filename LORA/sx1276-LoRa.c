...
{
    434000000,        // RFFrequency 434 МГц с поправкой
    2,                // Power
    ...
    true,             // ImplicitHeaderOn [0: OFF, 1: ON] todo??? работает наоборот
...
extern uint8_t CRCerror; // флаг ошибки CRC в пакете данных
uint8_t CRC_flag;
...
void SX1276LoRaInit( void )
{
    ...
    SX1276LoRaSetDefaults( ); // просто читаем ID чипа
    
    SX1276ReadBuffer( REG_LR_OPMODE, SX1276Regs + 1, 0x70 - 1 ); // читаем все регистры из модуля в структуру регистров
    
    SX1276LR->RegLna = RFLR_LNA_GAIN_G1;	// устанавливаем усиление LNA

    SX1276WriteBuffer( REG_LR_OPMODE, SX1276Regs + 1, 0x70 - 1 ); // записываем все регистры обратно
...
void SX1276LoRaSetDefaults( void )
{
    ...
    SX1276Read( REG_LR_VERSION, &SX1276LR->RegVersion ); // читаем версию , ID , регистр 0x42
}
...
void SX1276LoRaSetOpMode( uint8_t opMode )
{
    ...
    opModePrev = SX1276LR->RegOpMode & ~RFLR_OPMODE_MASK;  // текущий режим , из структуры регистров Lora

   // if( opMode != opModePrev )  // если новый режим не равен текущему
   // {
//        if( opMode == RFLR_OPMODE_TRANSMITTER ) // управление ключом антенны
//        {
...
//        }
    	...
    	SX1276LR->RegOpMode = ( SX1276LR->RegOpMode & RFLR_OPMODE_MASK ) | opMode; // новый режим в регистр RegOpMode по маске RFLR_OPMODE_MASK
...
SX1276Write( REG_LR_IRQFLAGS, 0xff );  //  дополнительный сброс всех флагов
...
SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags ); // читаем регистр флагов прерываний
if( ( SX1276LR->RegIrqFlags & RFLR_IRQFLAGS_PAYLOADCRCERROR ) == RFLR_IRQFLAGS_PAYLOADCRCERROR ) // проверка CRC
        {
			CRC_flag = 1; // программный флаг CRC
        }
            RxTimeoutTimer = GET_TICK_COUNT( );
//            if( LoRaSettings.FreqHopOn == true )
//            {
...
//printf("\n\r DIO0 == 1 0x12: 0x%02X  \n\r", LoraReadReg(0x12) );
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE );  // сброс флага

            while ( DIO0 == 1 ) // костыль
            {
            	SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE );  //  дополнительный сброс RFLR_IRQFLAGS_RXDONE
            }

//printf(" DIO0 == 1 0x12: 0x%02X  \n\r", LoraReadReg(0x12) );
//SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE );                                   // костыль, с первого раза не всегда сбраcывается флаг прерывания
//SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE );                                   // костыль, с первого раза не всегда сбраcывается флаг прерывания
//SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE );                                   // костыль, с первого раза не всегда сбраcывается флаг прерывания
//printf(" DIO0 == 1 0x12: 0x%02X  \n\r", LoraReadReg(0x12) );
//OffTest();
...
{
	SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_PAYLOADCRCERROR  ); // RFLR_IRQFLAGS_PAYLOADCRCERROR  СБРОС флага
	SX1276Read( REG_LR_IRQFLAGS, &SX1276LR->RegIrqFlags ); // читаем регистр флагов прерываний
}

            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_PAYLOADCRCERROR  ); // RFLR_IRQFLAGS_PAYLOADCRCERROR
//printf("PAYLOADCRCERROR 0x12: 0x%02X  \n\r", LoraReadReg(0x12) );
//SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_PAYLOADCRCERROR  );							// костыль, с первого раза не всегда сбраcывается флаг прерывания
//printf("PAYLOADCRCERROR 0x12: 0x%02X  \n\r", LoraReadReg(0x12) );
            //CplTest();
          /*  if( LoRaSettings.RxSingleOn == true ) // Rx single mode
            {
        ...
        {// вычисление SNR принятого пакета
        ...
        if( LoRaSettings.RxSingleOn == true ) // Rx single mode
        {
            ...
            if( LoRaSettings.ImplicitHeaderOn == true ) // Неявный
            {
                RxPacketSize = SX1276LR->RegPayloadLength;
                SX1276ReadFifo( RFBuffer, SX1276LR->RegPayloadLength );	// читаем данные из FIFO в буфер RFBuffer
            }
            else    // Если заголовок явный, берем длину буфера данных из него
            {
...
//printf("\tResult = %d (TX_INIT)\n\r", result );
        ...
        SX1276LoRaSetOpMode( RFLR_OPMODE_TRANSMITTER ); // старт передачи!
//OnTest();
...
while ( DIO0 == 1 ) // костыль todo разобраться!
  {
	SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_TXDONE );  //  дополнительный сброс RFLR_IRQFLAGS_TXDONE
  }
