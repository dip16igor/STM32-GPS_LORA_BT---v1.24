...
void ssd1306_I2C_Init() {
    ...
    i2c.I2C_ClockSpeed = 400000; // 400000
...
void ssd1306_I2C_WriteMulti(I2C_TypeDef* I2Cx, uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {
	uint16_t i;
	ssd1306_I2C_Start(I2Cx, address, I2C_TRANSMITTER_MODE, I2C_ACK_DISABLE); 	// формируем START на i2c шине
	ssd1306_I2C_WriteData(I2Cx, reg);											// запись в регистр reg
	for (i = 0; i < count; i++) {												// запись массива данных data[]
		ssd1306_I2C_WriteData(I2Cx, data[i]);
	}
	ssd1306_I2C_Stop(I2Cx);														// формируем STOP на шине
}
