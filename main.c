...
#define STOP \
	for (;;) \
	{        \
	} // бесконечный пустой цикл, для отладки
...
void init(void)
{
	...
	*/
	...
	if (GPSBuf[0] == '$' && GPSBuf[1] == 'P' && GPSBuf[2] == 'M' && GPSBuf[3] == 'T' && GPSBuf[4] == 'K' && GPSBuf[5] == '0' && GPSBuf[6] == '0' && GPSBuf[7] == '1' && GPSBuf[8] == ',' && GPSBuf[9] == '0' && GPSBuf[10] == ',' && GPSBuf[11] == '3' && GPSBuf[12] == '*' && GPSBuf[13] == '3' && GPSBuf[14] == '0') // скорость 9600
	{
		printf("\tOK  ");
		//			printf("\n\r Set to 115200 ");
		//			rprintfInit (GpsUsart);  				// переключаем вывод на USART3 - в модуль GPS
	...
	if (GPSBuf[0] == '$' && GPSBuf[1] == 'P' && GPSBuf[2] == 'M' && GPSBuf[3] == 'T' && GPSBuf[4] == 'K' && GPSBuf[5] == '0' && GPSBuf[6] == '0' && GPSBuf[7] == '1' && GPSBuf[8] == ',' && GPSBuf[9] == '0' && GPSBuf[10] == ',' && GPSBuf[11] == '3' && GPSBuf[12] == '*' && GPSBuf[13] == '3' && GPSBuf[14] == '0') // скорость 115200
		printf("\tOK  ");
	else
		printf("\tNO! ");
...
u16 GetSamplesADC1(void) // выборка АЦП
{
	while (ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC) == RESET)
		;							   // ждем флаг конца преобразования
...
void ReceiveGPS(void)
{
	...
	if (triggerTIM3Interrupt == 1) // проверка  - пришла посылка от GPS модуля
	{
		...
		if ((Fix == 2 || Fix == 3)) // если есть 2D или 3D решение
		{
			...
			if (GPSFixData.NS == 'N')
				GPS_N_S = 1; // признак северная-южная широта
			else
				GPS_N_S = 0;

			if (GPSFixData.EW == 'E')
				GPS_E_W = 0; // западная-восточная долгота
			else
				GPS_E_W = 1;
...
u8 NMEA_Parser_GSV_GPS(void)
{
	...
	while (!((GPSBuf[GSVStartPos] == '$') && (GPSBuf[GSVStartPos + 2] == 'P') // только GPS спутники
	...
	for (i = GSVStartPos + 5; i < GPSBUFLENGTH; i++) // Ждем или конца буфера или * в приемном буфере
	{
		...
		if (GPSBuf[i] == ',')
		{
			...
			CommaPos++;
			k = 0; // Наращиваем счетчик запятых
		}
		else
			switch (CommaPos) // Пользуемся тем, что данные разделяются запятыми. Неопределенные данные (две запятые подряд) мы заменили нулем в обработчике прерывания USART1_IRQHandler()
			...
			case 16:
				...
				break; // Номер спутника. case 4: case 8: case 12: case 16: - номера запятых,
...
u8 NMEA_Parser_GSV_GLO(void)
{
	...
	while (!((GPSBuf[GSVStartPos] == '$') && (GPSBuf[GSVStartPos + 2] == 'L') // только GLONASS спутники
			 && (GPSBuf[GSVStartPos + 3] == 'G') && (GPSBuf[GSVStartPos + 4] == 'S') && (GPSBuf[GSVStartPos + 5] == 'V')) &&
		   GSVStartPos < (GPSBUFLENGTH - sizeof("$GPGSV,n,n,nn,11,22,333,44,11,22,333,44,11,22,333,44,11,22,333,44*cs") + 1))
		GSVStartPos++;
	...
	for (i = GSVStartPos + 5; i < GPSBUFLENGTH; i++) // Ждем или конца буфера или * в приемном буфере
	{
		...
		if (GPSBuf[i] == ',')
		{
			...
			case 16:
				...
				break; // Номер спутника. case 4: case 8: case 12: case 16: - номера запятых, после которых идет соответствующая информация. В каждой GSV посылке есть информация о максимум 4-х спутниках, поэтому 20-ю запятую можно не обрабатывать
			case 5:
			case 9:
			case 13:
			case 17:
...
u8 ControlCheckSum(u16 StartIndex)
{
	...
	while (GPSBuf[i] != '*')
	{
		CheckSum ^= GPSBuf[i];
		if (++i == GPSBUFLENGTH)
			return 0; // Не найден признак контрольной суммы
	}

	if (GPSBuf[++i] > 0x40)
		MessageCheckSum = (GPSBuf[i] - 0x37) << 4; // Условие для корректной обработки десятичных
	else
		MessageCheckSum = (GPSBuf[i] - 0x30) << 4; // и шестнадцатиричных чисел, представленных в коде ASCII
	if (GPSBuf[++i] > 0x40)
	...
	if (MessageCheckSum != CheckSum)
		return 0; // Неправильное значение контрольной суммы
...
void LoraWriteBuffer(u8 Addr, u8 *buffer, u8 size)
{
	...
	while (SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET)
		; // ждем пока освободится буфер передачи
...
void LoraReadBuffer(u8 Addr, u8 *buffer, u8 size)
{
	...
	while (SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) == RESET)
		;					   // ждем пока байт прочитается целиком
